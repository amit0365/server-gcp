import {
  ...
  import { CIRCOM_FIELD_MODULUS, MAX_HEADER_PADDED_BYTES, MAX_BODY_PADDED_BYTES, STRING_PRESELECTOR } from "./constants";
  import { shaHash, partialSha, sha256Pad } from "./shaHash";
  ...
  async function getArgs() {
    ...
    const email_file = emailFileArg ? emailFileArg.split("=")[1] : "./src/input_gen/test_sendgrid.eml";
  ...
  export async function getCircuitInputs(
    ...
    const in_blinding = Array(32).fill("0");
  ...
  async function do_generate(writeToFile: boolean = true) {
    ...
    if (writeToFile) {
      ...
      const filename = "./src/proof_gen/twitter3sha_input.json";
